...
@WebServlet("/pay")
public class ReadyPayServlet extends HttpServlet {
    @Override
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        //获取输入的参数并解析
        String goodsIdAndNum = req.getParameter("goodsIdAndNum");//1-2,38-10
        String[] strings = goodsIdAndNum.split(",");
        List<Goods> goodsList = new ArrayList<>();
        for(String s : strings){
            String[] strs = s.split("-");
            Goods goods = this.getGoods(Integer.parseInt(strs[0]));
            if(goods != null){
                int buyNum = Integer.parseInt(strs[1]);
                int haveNum = goods.getStock();
                if(haveNum >= buyNum){
                    goods.setPayGoodsNum(buyNum);
                    goodsList.add(goods);
                }else {
                    Writer writer = resp.getWriter();
                    writer.write("<h2>此货物不足</h2>");
                    throw new RuntimeException("此货物不足");
                }
            }else{
                Writer writer = resp.getWriter();
        ...
        for(Goods goods:goodsList){
            ...
            orderItem.setGoods_num(goods.getPayGoodsNum());
            ...
            //计算当前价格
            int currentPrice = goods.getPayGoodsNum() * goods.getPriceInt();
            CountPrice += currentPrice;
            //计算折扣后价格
            int discountPrice = currentPrice * goods.getDiscount()/100;//折扣记录的是分，需要除以100
